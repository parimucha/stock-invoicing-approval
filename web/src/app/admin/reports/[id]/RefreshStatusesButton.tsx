...
import { refreshJiraStatuses, type RefreshStatusesResult } from "./actions";

type Props = {
  reportId: number;
};

export function RefreshStatusesButton({ reportId }: Props) {
  const [state, action, pending] = useActionState<RefreshStatusesResult | null, FormData>(
    refreshJiraStatuses,
    null,
  );
  ...
  return (
    <form action={action} className="flex items-center gap-3 flex-wrap">
      ...
      >
        {pending ? "Refreshing from JIRA…" : "Refresh JIRA statuses"}
      </button>
...
function ResultText({ state }: { state: RefreshStatusesResult }) {
  if (!state.ok) {
    return (
  ...
  if (state.missingFromJira > 0) {
    parts.push(`${state.missingFromJira} not in JIRA`);
  }
  return (
